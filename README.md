...
The port is trying to be a simple, straightforward, 1:1 with the least possible modifications done to the original Starterkit in order to serve as a nice starting point for people wanting to use Vue.js with Kirby.
...
Deploy `backend` and `frontend` folders to the same server at corresponding public paths.
